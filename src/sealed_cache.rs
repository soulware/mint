...
//! content-addressed and `ls`/`cat`-inspectable; nothing here is precious
//! or authoritative on its own.
//!
...
impl ServedSurface {
    ...
    /// return the `ServedSurface` to serve. The policy bytes are taken from
    /// `config`, which the caller has already established satisfies `seal` —
    /// the startup adopt path (templates hash-match a verified bucket seal)
    /// or the seal handler (the seal was just authored from this same
    /// config). This is the tail both share once they hold a known-good
    /// seal: write the cache, hand back the surface.
    pub fn materialize(config: &Config, seal: &Seal, data_dir: &Path) -> Result<Self, SealError> {
        let templates = policies_from_config(config);
        write(data_dir, seal, &templates)?;
        Ok(ServedSurface {
            seal: seal.clone(),
            templates,
        })
...
"#;
        crate::config::parse_for_test(
            toml,
            &[
                ("volume-ro.json", r#"{"ro":"demo-bucket"}"#),
                ("volume-rw.json", r#"{"rw":"demo-bucket"}"#),
            ],
        )
        .expect("parse")
    }
    ...
    fn write_then_load_round_trips_every_policy() {
        ...
        match load(dir.path()).expect("load") {
            CacheState::Loaded {
                ...
                assert_eq!(got.get("volume-ro").unwrap(), r#"{"ro":"demo-bucket"}"#);
                assert_eq!(got.get("volume-rw").unwrap(), r#"{"rw":"demo-bucket"}"#);
            }
