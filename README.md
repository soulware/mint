...
Policy templates support expressions resolved from configuration `{{env.X}}` and from caveats on the macaroon itself `{{caveat.X}}`. A derived expression `{{mint.expiry}}` that resolves to an expiry timestamp is also supported.
